...
final class ClientIdType extends StringType
{
     ...
     */
    public function convertToPHPValue($value, AbstractPlatform $platform): ?ClientIdValueObject
    {
        if ($value === null) {
            return null;
        }
        ...
        try {
            return new ClientIdValueObject((string)$value);
    ...
    {
        if (!($value instanceof ClientIdValueObject)) {
            throw ConversionException::conversionFailedInvalidType(
                $value,
                __CLASS__,
                [ClientIdValueObject::class]
